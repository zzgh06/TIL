...
### **💻 금일 회고**
    오늘 강의 내용 자체는 어렵진 않았지만, 문제 자체를 이상하게 이해하여 한참을 헤마다가 결국 1문제 밖에 풀지못했다...
    그것이 아니라도 하루 쉬었다고 HTML, CSS를 거의 까먹은 듯하다, 새삼 프론트엔드의 무시무시함을 깨닫는 요즘이다.
    오늘은 3월의 새로운 방식으로 진행되는 햇님반 스터디 모임을 진행하였다, 확실히 회의내용되로 기초를 다지기에
    백준 및 SWEA 알고리즘 풀이는 시간적 활용능력이 다소 떨어진다고 느꼈는데, 다들 마찬가지였나보다.
    그래서 새로운 알고리즘 풀이사이트를 이용하여 기본부터 알고리즘을 풀이를 진행하기로 하여 오늘은 해당 사이트인
    코드트리 사이트에 방문하여 기본문제 23문제 정도 풀었다.
