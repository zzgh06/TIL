...
끝입니다. 그러면 아마 count라는게 2일 때 까지 실행해주니까 age는 20에서 22가 되면 더이상 증가하지 않고 멈추겠군요.
...
근데 지금은 count가 3일 때도 age +1를 해주고 있는 듯 합니다. 왜죠? 
...
이 모든 문제는 `setCount()가 async 함수`라서 그렇습니다. 
...
근데 뒤에다가 [] 대괄호안에 state를 집어넣으면 state가 변경되면 이 코드 실행해주세요~ 라는 뜻으로도 사용가능합니다.
